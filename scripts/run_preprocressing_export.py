...
def run_preprocessing_export(file):
    ...
    subprocess.run(f"export S3_BASE={s3_base_dir}", shell=True)
    subprocess.run(f"python open_lm/datapreprocess/make_2048.py --input-files {file} --output-dir /scratch/kjablonk/preproc_data --num-workers 30 --num-consumers 8 --upload-to-s3", shell=True)
